...
public class InterceptorProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        if (type != null && !hasDone) {
            try {
                builder(com.android.router.compiler.CompilerConstant.AutoCreateInterceptorPackage, type).build().writeTo(mFiler);
                hasDone = true;
            } catch (Exception e) {
                error("wrong!!:" + e.getMessage());
            }
    ...
    private TypeSpec getInterceptorInitializer(Set<? extends Element> interceptorElements) throws ClassNotFoundException {
        ...
        return TypeSpec.classBuilder(com.android.router.compiler.CompilerConstant.AutoCreateInterceptorPrefix + moduleName)
                .addModifiers(Modifier.PUBLIC)
